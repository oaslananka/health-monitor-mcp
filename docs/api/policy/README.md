[**health-monitor-mcp v1.1.0**](../README.md)

***

[health-monitor-mcp](../README.md) / policy

# policy

## Interfaces

- [RuntimePolicy](interfaces/RuntimePolicy.md)
- [RuntimePolicyOptions](interfaces/RuntimePolicyOptions.md)

## Type Aliases

- [RuntimeTransport](type-aliases/RuntimeTransport.md)

## Variables

- [STDIO\_DISABLED\_MESSAGE](variables/STDIO_DISABLED_MESSAGE.md)
- [STDIO\_COMMAND\_UNSAFE\_MESSAGE](variables/STDIO_COMMAND_UNSAFE_MESSAGE.md)
- [STDIO\_COMMAND\_NOT\_ALLOWED\_MESSAGE](variables/STDIO_COMMAND_NOT_ALLOWED_MESSAGE.md)

## Functions

- [createRuntimePolicy](functions/createRuntimePolicy.md)
